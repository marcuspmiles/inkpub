...
import { saveArticle, unsaveArticle } from "@/server/engagement";

type Context = { params: Promise<{ id: string }> };

const MESSAGE = "Sign in to save articles.";
...
export const POST = withErrorHandling(async (request: Request, context: Context) => {
  ...
  const result = await saveArticle(actor.user.id, actor.id);
...
export const DELETE = withErrorHandling(async (request: Request, context: Context) => {
  ...
  return apiSuccess(await unsaveArticle(actor.user.id, actor.id));
});
